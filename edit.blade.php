...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Dosen</h1>
            <p class="text-sm text-slate-500">Perbarui informasi profil dosen pengampu.</p>
        </div>
    </div>
        ...
        <form action="{{ route('dosen.update', $dosen->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="npp" name="npp" value="{{ old('npp', $dosen->username) }}" placeholder="Contoh: 1974020101" class="w-full pl-11 pr-4 py-3 bg-slate-50 border @error('npp') border-rose-300 focus:border-rose-500 @else border-slate-200 focus:border-brand-500 @enderror rounded-xl text-sm focus:outline-none focus:bg-white transition-all">
                </div>
                @error('npp')
